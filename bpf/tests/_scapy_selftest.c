...
const unsigned char sst_large[] = {
	SCAPY_BUF_BYTES(sst_large)
};

const unsigned char sst_xl[] = {
	SCAPY_BUF_BYTES(sst_xl)
};
...
int pktgen_scapy_large_pkts(struct __ctx_buff *ctx)
{
	...
	scapy__push_data(&builder, (void *)sst_large, sizeof(sst_large));
...
int check_scapy_large_pkts(struct __ctx_buff *ctx)
{
	...
	ASSERT_CTX_BUF_OFF("assert_large_1", "Ether", ctx, 0, sst_large,
			   sizeof(sst_large));
...
int pktgen_scapy_xlarge_pkts(struct __ctx_buff *ctx)
{
	...
	scapy__push_data(&builder, (void *)sst_xl, sizeof(sst_xl));
...
int check_scapy_xlarge_pkts(struct __ctx_buff *ctx)
{
	...
	ASSERT_CTX_BUF_OFF("assert_xl_1", "Ether", ctx, 0, sst_xl,
			   sizeof(sst_xl));
